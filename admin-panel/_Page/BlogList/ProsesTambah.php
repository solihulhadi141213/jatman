<?php
    ...
    // Validasi input
    $requiredFields = [
        'title_blog'  => 'Judul Blog Tidak Boleh Kosong!',
        'tag'         => 'Tag Blog Tidak Boleh Kosong!',
        'deskripsi'   => 'Deskripsi Blog Tidak Boleh Kosong!',
        'author_blog' => 'Penulis Blog Tidak Boleh Kosong!'
    ];
    ...
    // Variabel Input
    $title_blog  = validateAndSanitizeInput($_POST['title_blog']);
    $tag         = validateAndSanitizeInput($_POST['tag']);
    $deskripsi   = validateAndSanitizeInput($_POST['deskripsi']);
    $author_blog = validateAndSanitizeInput($_POST['author_blog']);

    // Status Publish (default 0)
    $publish = !empty($_POST['publish']) ? (int) $_POST['publish'] : 0;

    // Ubah tag string ke array
    $tagsArray = array_map('trim', explode(',', $tag));
    $tagsArray = array_values(array_filter($tagsArray));
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL            => $base_url . '/_Api/InsertBlog.php',
