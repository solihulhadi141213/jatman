<?php
    ...
    // Validasi input
    $requiredFields = [
        'submenu_order_id'  => 'Order ID Tidak Boleh Kosong!',
        'menu_order_id2'    => 'Order Menu ID Tidak Boleh Kosong!'
    ];
    ...
    //Buatkan Variabelnya
    $submenu_order_id=validateAndSanitizeInput($_POST['submenu_order_id']);
    $menu_order_id=validateAndSanitizeInput($_POST['menu_order_id2']);

    $curl = curl_init();

    curl_setopt_array($curl, array(
    CURLOPT_URL => ''.$base_url.'/_Api/DeleteSubMenu.php',
    ...
    CURLOPT_POSTFIELDS =>'{
        "order-parent":'.$menu_order_id.',
        "order-child":'.$submenu_order_id.'
    }',
    ...
    if($status_response=="success"){
        echo '<span class="text-success" id="NotifikasiDeleteSubmenuBerhasil">Success</span>';
        exit();
    }else{
